...
export const ids = {
  ...
  formalActa: 'formal-acta-1',
  formalToken: 'formal-token-1',
};
...
export async function seedSecurityData(testEnv) {
  ...
  await testEnv.withSecurityRulesDisabled(async (context) => {
    ...
    await Promise.all([
      ...
      setDoc(doc(db, 'actas_formales', ids.formalActa), {
        titulo: 'Acta formal de prueba',
        fecha: '2026-03-12',
        hora: '09:00',
        lugar: 'Sala de juntas',
        tipoReunion: 'Comite',
        asistentes: [{ id: 'asistente-1', nombre: 'Firmante Uno', cargo: 'Profesional', token: ids.formalToken }],
        objetivo: 'Validar reglas',
        ordenDia: ['Prueba'],
      ...
      setDoc(doc(db, 'actas_formales', ids.formalActa, 'firmantes', ids.formalToken), {
        actaId: ids.formalActa,
        asistenteId: 'asistente-1',
        tituloActa: 'Acta formal de prueba',
        fechaActa: '2026-03-12',
        tipoReunion: 'Comite',
        nombre: 'Firmante Uno',
        cargo: 'Profesional',
        estado: 'pendiente',
