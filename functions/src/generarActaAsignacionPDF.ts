...
export async function generarActaAsignacionPDF({
  ...
  asignacion,
  storage,
}: GenerarPDFParams): Promise<Buffer> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      doc.fontSize(10).font('Helvetica');
      doc.text(
        'El profesional de logistica certifica que realizó la entrega inicial del activo descrito y que la información registrada corresponde al estado en que fue entregado.',
        { align: 'justify' }
      );
      doc.moveDown(0.5);
      doc.text(
        'El custodio certifica que recibe el activo descrito, acepta la responsabilidad sobre su uso y conservación, y declara que la información registrada es veraz.',
        { align: 'justify' }
      );
      doc.moveDown(2);
